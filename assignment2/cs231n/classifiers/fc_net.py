...
class TwoLayerNet(object):
	...
	def loss(self, X, y=None):
		...
		dout,grads['W2'], grads['b2'] = affine_backward(d_scores,cache2)
		grads['W2'] += self.reg * self.params['W2']
		dout = relu_backward(dout,cache1_a)
		_,grads['W1'], grads['b1'] = affine_backward(dout,cache1)
...
class FullyConnectedNet(object):
		...
		for layer_number in range(self.num_layers):
			...
			if layer_number == 0:
				...
				if self.use_batchnorm:
					self.params['gamma' + str(layer_number + 1)] = np.ones(hidden_dims[layer_number],)
					self.params['beta' + str(layer_number + 1)] = np.zeros(hidden_dims[layer_number],)
	...
	def loss(self, X, y=None):
		...
		for layer_number in range(self.num_layers):
			...
			# don't apply relu and batch norm to last layer
